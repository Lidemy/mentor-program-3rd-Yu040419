<?php
  ...
  echo   "<div class='comment__block card mb-4'>";
  echo     "<div class='card-body'>";
  echo       "<div class='comment__block--info mb-1 d-flex justify-content-between align-items-center'>";

  // 如果是使用者的留言，有編輯及刪除選單
  if ($row['username'] === $username) {

    echo  "<div class='d-flex align-items-center'>";
    echo    "<span class='mb-0 card-title h5 comment__username'>" . escape($row['username']) ."</span>";
    echo  "</div>";
    ...
    echo      "<div class='comment__delete dropdown-item' data-id='". escape($row['comm_id']) ."' data-type='comment'>刪除</div>";
    echo    "</div>";
    echo  "</div>";
  
  } else {
    echo  "<div class='d-flex align-items-center'>";
    echo    "<span class='mb-0 card-title h5'>" . escape($row['username']) ."</span>";
    echo  "</div>";
  }
  
  echo  "</div>";
  echo  "<div class='comment__info--time mb-2'>" . escape($row['time']) . "</div>";

  $text = sprintf(
    "<p class='comment__text card-text' data-id='%d' parent-id='0'>%s</p>",
    escape($row['comm_id']),
    escape($row['text'])
  );
  ...
  // 新增子留言
  echo  "<form class='subcomment__form d-flex flex-column mt-2' action ='./handle_add_comment.php' method='POST' >";
  ...
  // 如果已登入
  if (!empty($_SESSION['username'])) {
    echo  "<input type='button' class='button add__comment mt-3' value='我要回覆' />";
  // 如果未登入
  } else {
    echo  "<a href='./login.php' class='button mt-2'>我要登入</a>";
  }
  echo  "</form>";
